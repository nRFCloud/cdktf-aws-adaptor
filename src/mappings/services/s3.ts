  ...
  registerMappingTyped(CfnBucket, S3Bucket, {
        ...
        new S3BucketNotification(
          ...
          deleteUndefinedKeys({
            ...
            queue: props?.NotificationConfiguration?.QueueConfigurations?.map(queueConfig => ({
              ...
              filterPrefix: queueConfig?.Filter?.S3Key?.Rules?.find(rule => rule.Name === "prefix")?.Value as string,
              filterSuffix: queueConfig?.Filter?.S3Key?.Rules?.find(rule => rule.Name === "suffix")?.Value as string,
            })),
            lambdaFunction: props?.NotificationConfiguration?.LambdaConfigurations?.map(lambdaConfig => ({
              events: [lambdaConfig.Event],
              filterPrefix: lambdaConfig?.Filter?.S3Key?.Rules?.find(rule => rule.Name === "prefix")?.Value as string,
              filterSuffix: lambdaConfig?.Filter?.S3Key?.Rules?.find(rule => rule.Name === "suffix")?.Value as string,
            ...
            topic: props?.NotificationConfiguration?.TopicConfigurations?.map(topicConfig => ({
              ...
              filterPrefix: topicConfig?.Filter?.S3Key?.Rules?.find(rule => rule.Name === "prefix")?.Value as string,
              filterSuffix: topicConfig?.Filter?.S3Key?.Rules?.find(rule => rule.Name === "suffix")?.Value as string,
            })),
          }),
        );
      }
        ...
        new S3BucketLoggingA(
          ...
          deleteUndefinedKeys({
            ...
            targetObjectKeyFormat: {
              ...
              partitionedPrefix: {
                partitionDateSource: props?.LoggingConfiguration?.TargetObjectKeyFormat?.PartitionedPrefix
          ...
          deleteUndefinedKeys({
            ...
            rule: props?.LifecycleConfiguration?.Rules?.map(rule => ({
              abortIncompleteMultipartUpload: {
                daysAfterInitiation: rule?.AbortIncompleteMultipartUpload?.DaysAfterInitiation as number,
              },
              expiration: {
              ...
              noncurrentVersionExpiration: {
                newerNoncurrentVersions: rule?.NoncurrentVersionExpiration?.NewerNoncurrentVersions?.toString(),
              ...
              noncurrentVersionTransition: rule?.NoncurrentVersionTransition
                ? [{
                  ...
                  newerNoncurrentVersions: rule?.NoncurrentVersionTransition?.NewerNoncurrentVersions?.toString(),
              ...
              filter: {
                prefix: rule?.Prefix,
                and: {
                  tags: Object.fromEntries(rule.TagFilters?.map(filter => [filter.Key, filter.Value]) || []),
                },
    ...
    attributes: {
      ...
      WebsiteUrl: (bucket: S3Bucket) => {
        const website = (bucket as unknown as { [BUCKET_WEB_SITE]: S3BucketWebsiteConfiguration })[BUCKET_WEB_SITE];
