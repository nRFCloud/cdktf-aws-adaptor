...
export function registerRoute53Mappings() {
    registerMappingTyped(CfnRecordSet, Route53Record, {
        resource(scope, id, props) {
            const mappedProps: Omit<typeof props, "Ttl"> & {
                TTL: (typeof props)["Ttl"];
            } = props as never;
            return new Route53Record(
                ...
                deleteUndefinedKeys({
                    alias: {
                        ...
                        evaluateTargetHealth: (mappedProps.AliasTarget
                            ? mappedProps.AliasTarget.EvaluateTargetHealth || false
                    ...
                    geoproximityRoutingPolicy: {
                        bias: mappedProps.GeoProximityLocation?.Bias as number,
                        awsRegion: mappedProps.GeoProximityLocation?.AWSRegion as string,
                        coordinates: [{
                            latitude: mappedProps.GeoProximityLocation?.Coordinates?.Latitude as string,
                            longitude: mappedProps.GeoProximityLocation?.Coordinates?.Longitude as string,
                        }],
                        localZoneGroup: mappedProps.GeoProximityLocation?.LocalZoneGroup as string,
                    },
